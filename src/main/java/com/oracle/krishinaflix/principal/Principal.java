...
//        }
        ...
        List<DadosEpisodio> dadosEpisodioList = temporadas.parallelStream()
                .flatMap(t -> t.episodiosList().stream())
                .collect(Collectors.toList());

        //.toList() -> Gera uma lista que não pode ser modificada
//
//        dadosEpisodioList.parallelStream()
...
//        firstOutro.forEach(System.out::println);
        ...
        avaliacoesPorTemporada.forEach((temporada,media) -> {
            System.out.println("Temporada= " + temporada + " | Média= " + df.format(media));
        });
